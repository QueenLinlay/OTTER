...
std::string windowTitle = "Assignment 1 - Justin Lee - 100658626";
...
int main() {
	...
	// Static transform
//	transform = glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0, 0));
	...
	VertexArrayObject::Sptr vao4 = ObjLoader::LoadFromFile("Fixedweirdobj.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (NotOrth) {
			camera->SetOrthoVerticalScale(1.0f);
		...
		// Rotate our models around the y axis
		transform2  = glm::rotate(glm::mat4(1.0f), static_cast<float>(thisFrame), glm::vec3(0, 1, 0)); // for assignment rotate y
		...
		transform = glm::translate(glm::mat4(1.0f), glm::vec3(1.0f, 0.0f, 0.0f)); // move object to left 1
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection() * transform * transform2);
		vao4->Draw();

		transform = glm::translate(glm::mat4(1.0f), glm::vec3(-1.0f, 0.0f, 0.0f)); // move object to right 1
		shader->SetUniformMatrix("u_ModelViewProjection", camera->GetViewProjection() * transform * transform2);
		vao4->Draw();
